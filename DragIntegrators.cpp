...
void integrate_drag_RK(std::vector<Cell> &c, Params p, double dt)
{
    ...
    if(p.DragIntegrator == 1){
        ...
        if(dt < ts_max){
            ...
            b = 0.;
            beta2 = 0.5 - gamma2;
            beta1 = (gamma1+gamma2-2*gamma1*gamma2-1)/ (2*gamma2-1.);
        }
    }else{
        ...
        gamma2 = 3. - gamma1;
        beta2 = gamma1 - 4.;
        beta1 = (gamma1*gamma1 - 3*gamma1 + 4.) / (4. - gamma1);
        ...
        if(dt < ts_max){
            ...
            b = 0.;
            beta2 = 0.5 - gamma2;
            beta1 = (10*gamma1*gamma2 - 5*gamma1 - 5*gamma2 + 7)/ (5. - 10*gamma2);
        }
    }
    ...
    void MDIRK_K(Cell &c, Params p, double dt, double gamma){
        ...
        for(int j=0; j<p.N_dust;j++){
            idust = j+1;
            c.alpha[j] = p.K[j] / c.U[idust][0];
            Ax += (c.U[idust][1] * c.alpha[j]) / (1. + gamma * dt * c.alpha[j]);
            B += c.U[idust][0] / c.U[0][0] * c.alpha[j] / (1. + gamma * dt * c.alpha[j]);
        }
    
        c.K[0][0] = 0.;
        c.K[0][1] = (Ax - c.U[0][1]*B) / (1. + gamma * dt * B);
        c.K[0][2] = 0.;
    
        for(int j=0; j<p.N_dust;j++){
            idust = j+1;
            c.K[idust][0] = 0.;
            c.K[idust][1] = c.alpha[j] / (1.+gamma*dt * c.alpha[j]) * ((c.U[idust][0]/c.U[0][0] * c.U[0][1] - c.U[idust][1])  +   gamma*dt*c.U[idust][0]/c.U[0][0] * c.K1[0][1]);
        }
    }
